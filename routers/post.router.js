...
postRouter.get("/",async(req,res)=>{
    try {
        ...
        let post;
        if(device){
            post=await PostModel.find({userID:req.body.userID,device},)
        }
        else{
            post=await PostModel.find({userID:req.body.userID})
        }
        res.status(200).send({"msg":"data fetched successfully","post":post})
    } catch (error) {
        res.status(400).send({"error":error})
    }
})
